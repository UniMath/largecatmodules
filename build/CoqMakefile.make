...
# coq_makefile -f Make -o .coq_makefile_output 
#
...
COQLIBS?=\
  -Q "." Largecat\
  -R "TypeTheory/TypeTheory/" TypeTheory\
  -R "UniMath/UniMath/" UniMath
COQCHKLIBS?=\
  -R "." Largecat\
  -R "TypeTheory/TypeTheory/" TypeTheory\
  -R "UniMath/UniMath/" UniMath
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/Largecat/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/Largecat/$$i; \
	done
...
uninstall_me.sh: .coq_makefile_output
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Largecat && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Largecat" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
